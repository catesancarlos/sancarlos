...
export default function CatLiturgia({ datos }){
    ...
    const handleSelect = (e) => {
        ...
        if(datos?.respuestas[e] == datos?.correcto){
            ...
            audio.play()
            if(footer) updateDoc(doc(db, 'concurso2conf', grupos[now-1].id), { puntos: +grupos[now-1].puntos + 20 })
            setRevisar('CORRECTO')
        }
        else{
...
export async function getServerSideProps (context) {
    ...
    const datos = await getDoc(doc(db, 'js-liturgia', `js-lt-${pregunta}`)).then(datos => {
        return (datos.data())
    })
