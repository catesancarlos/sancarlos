...
export default function PosBiblicoFem({
    ...
    useEffect(() => {
        const q = query(collection(db, 'equipos2026'), where('nivel', '==', 'biblico'), where('genero', '==', 'F'))
    ...
    useEffect(() => {
        setLoading(true)
        const qPartidos = query(collection(db, 'partidos2026'), where('grupo', '==', 'Bib'), where('genero', '==', 'F'))
    ...
    return (
        <section>
            <div className='tables'>
                ...
                <div style={{ width: '100%' }}>
                    {partidos?.filter(f => f.fase === 'FINAL' && f.grupo === 'Bib').map(p => (
                        <ItemCalendario
                            key={p.id}
