...
export default function PosComuFem() {
    ...
    useEffect(() => {
        const q = query(collection(db, 'equipos2026'), where('nivel', '==', 'comunion'), where('genero', '==', 'F'))
    ...
    useEffect(() => {
        setLoading(true)
        const qPartidos = query(collection(db, 'partidos2026'), where('grupo', '==', 'Com'), where('genero', '==', 'F'))
