...
export default function PosRecMas() {
    const [equipos, setEquipos] = useState([])
    const [partidos, setPartidos] = useState([])
    const [loading, setLoading] = useState(true)

    useEffect(() => {
        const q = query(collection(db, 'equipos2026'), where('nivel', '==', 'reconciliacion'), where('genero', '==', 'M'))
    ...
    useEffect(() => {
        setLoading(true)
        const qPartidos = query(collection(db, 'partidos2026'), where('grupo', '==', 'Rec'), where('genero', '==', 'M'))
